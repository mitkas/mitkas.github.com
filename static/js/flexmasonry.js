    ...
    function f(e) {
      ...
      if (Array.from(n).length !== d() - 1)
        for (let n = 1; n < d(); n++) {
          ...
          t.classList.add("flexmasonry-break"),
            t.classList.add("flexmasonry-break-" + n),
            e.appendChild(t);
        }
    }
    function u(e) {
      e.classList.contains("flexmasonry-cols-" + d()) ||
        ((e.className = e.className.replace(/(flexmasonry-cols-\d+)/, "")),
        e.classList.add("flexmasonry-cols-" + d()));
    }
    function d() {
